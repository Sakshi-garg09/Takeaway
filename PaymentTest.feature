...
Feature: Payment Option 
  ...
  Scenario Outline: All payment options should be backend configurable to disable
    ...
    Examples:
      ...
      @manual @deep @e2e.android @e2e.ios @target-payment-api
  Scenario Outline: Check the Database entries whether they store credit card details or not
    Given User is on payment method screen
    And User select any one the payment method
    And User has saved card in card tab
    And User click on "Pay" button to proceed
    Then check the entries in database whether it stored the credit card details or not
   
   
   
    @manual @e2e.android @e2e.ios
  Scenario Outline: Check confirmation page is shown to the user after successful transaction
    Given User is on payment method screen
    And User select any one the payment method
    And User navigate to card details page
    And User enter all the details of card
    When User click on "Pay" button to proceed
    Then Confirmation message should be shown to the user
